...
const PairingPage = () => {
    ...
    useEffect(() => {
        const { request, abort } = api("get", `/pair/pairs`);
        const getData = async () => {
            try {
    ...
    const handleUnlink = async (pair) => {
        ...
        try {
            const { request } = api("delete", `/pair/delete?id=${pair._id}`);
            await request;
            showMessage("Pair unlinked successfully", "success");
            setPairings(prev => prev.filter(p => p._id !== pair._id));
            ...
            if (pair.learner) {
                ...
                if (rating !== null) {
                    ...
                    const { request } = api("post", `/account/rating`, { student_id: otherUser, rating });
                    await request;
    ...
    const handleDownload = (pair) => {
        ...
        const UID = `${crypto.randomUUID()}-${pair._id}@teachandtackle`;
            ...
            <div className={styles.pairing_container}>
                ...
                {pairings && pairings.map(pair => (
                    <div className={styles.pair} key={pair._id}>
                        ...
                        <Student student={pair.receiver_info} classes={styles.student} />
                    </div>
                ))}
            </div >
        </>
